...
## default port prefixes for cygnusd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/cygnusd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/cygnusd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/cygnusd add-genesis-account \
    "$("$PWD"/build/cygnusd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000acygnus,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/cygnusd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/cygnusd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/cygnusd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/cygnusd start --pruning=nothing --rpc.unsafe \
