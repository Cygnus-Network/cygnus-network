...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("cygnus", "cygnuspub")
}
...
func TestGetReceivedCoin(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			"transfer",
			"channel-0",
			"transfer/channel-0/acygnus",
			"10",
			sdk.Coin{Denom: "acygnus", Amount: sdk.NewInt(10)},
		},
		{
...
func TestGetSentCoin(t *testing.T) {
	testCases := []struct {
		...
		{
			"get unwrapped acygnus coin",
			"acygnus",
			"10",
			sdk.Coin{Denom: "acygnus", Amount: sdk.NewInt(10)},
		},
		{
			"get ibc wrapped acygnus coin",
			"transfer/channel-0/acygnus",
			"10",
