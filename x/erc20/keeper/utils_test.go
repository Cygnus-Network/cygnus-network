...
func (suite *KeeperTestSuite) DoSetupTest(t require.TestingT) {
	...
	header := testutil.NewHeader(
		1, time.Now().UTC(), "cygnus_96001-1", consAddress, nil, nil,
	)
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	// Mint coins on the osmosis side which we'll use to unlock our acygnus
	...
	// Mint coins on the cosmos side which we'll use to unlock our acygnus
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/acygnus") as input parameter.
